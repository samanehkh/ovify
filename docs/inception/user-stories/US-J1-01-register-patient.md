...
## 1. User story
> As **Mona (the clinic nurse)**, I want to **manually input patient bio details, select a treatment package, set their next scan appointment, configure their daily medications, and connect their support partner on my tablet**, so that **I can register the patient and trigger their onboarding companion app setup in under 60 seconds.**
...
- **Exit:** On success, redirects to the Triage Dashboard with a confirmation toast; invitation SMS is sent to the patient.

## 3. Acceptance criteria (Gherkin)
```gherkin
Scenario: Successful patient registration with predefined package and common medication
  ...
  And selects Cycle Start Date "2026-07-12" and Current Cycle Number "1"
  ...
  And selects Next Appointment Date "2026-07-15" and Time "09:00"
  And adds a medication: taps Gonal-F card, selects dosage "150 IU", route "Subcutaneous", scheduled time "19:00", start date "2026-07-12"
  And taps "Register Patient"
  Then the system saves the patient chart with current_cycle_number=1
  And sends the onboarding SMS to Sarah
  And displays success toast: "Sarah Khan registered successfully. SMS invite sent."

Scenario: Registering with a custom package
  Given Mona is filling the registration form
  When she selects Treatment Package "Other (Custom)"
  Then the system displays a text input: "Custom Package Name"
  When she enters "PGT-A Screening Add-on" and completes registration
  Then the database saves the package type as Other and registers custom name "PGT-A Screening Add-on"

Scenario: Adding uncommon medication via dropdown
  Given Mona is building the stimulation protocol
  When she types "Rekovelle" in the autocomplete search bar
  And selects "Rekovelle" from the auto-suggested options
  Then the medication row is added for Rekovelle with dynamic dosage, route, time, and start date selectors
```
...
*   **Main Navigation:**
    ...
    4.  **Formulary Setup:** SME tool to configure clinic-approved drugs and standard dosages.
    5.  **Clinician Management:** List of active staff profiles and access levels.
...
2.  **Section 2: Treatment & Cycle Setup**
    ...
    *   Current Cycle Number: Segmented touch selection `[1] [2] [3]`.
...
4.  **Section 4: Next Appointment**
    *   Date Picker and Time Picker (custom touch wheel for hours/minutes).
5.  **Section 5: Stimulation Protocol Builder**
    ...
    *   **Prescription List:** Tapping a medication adds a card with:
        *   Medication Name.
        *   Dosage: Dropdown (`75 IU`, `150 IU`, `225 IU`, `300 IU`, `0.25mg`, `250 mcg`, or `Custom`).
        *   Route: Dropdown (`Subcutaneous`, `Intramuscular`).
        *   Scheduled Time: Dropdown (30-minute intervals).
        *   Start Date: Date picker (default: Cycle Start Date).
        *   Action: `[Delete]` button (Red trash icon).
6.  **Section 6: Action Panel (Sticky Bottom Bar)**
    *   `[Cancel]`: Triggers validation dialog: *"Discard patient registration draft? All unsaved data will be lost."*
    *   `[Turn Screen to Patient]`: Opens a modal displaying only Sarah's contact details in massive font: *"Please verify your contact details for security: Phone: +971501234567, Email: sarah@example.com. [Confirm & Close]"*
    *   `[Register Patient]`: Submits the payload. Shows spinner and disables when clicked.

---

| State | Trigger | What the user sees |
|---|---|---|
| **loading** | Clicked Register | Spinner overlay indicating database prep. |
| **ready** | Default load | Blank form sections, left sidebar navigation active. |
| **submitting** | Register tapped | Button shows spinner + "Registering..."; inputs disabled. |
| **success** | 200 OK | Leaves to Triage Dashboard; toast "Registered successfully." |
| **error (recoverable)** | 400 Duplicate | Red inline alert banner: "Phone number already registered." |
...
| current_cycle_number| input | integer | Y | `1`, `2`, or `3` | default = 1 |
...
## 7. Components
- Shared navigation sidebar, numeric segmented buttons, hybrid autocomplete grid card select.
...
| cancel_warning | "Discard patient registration draft? All unsaved data will be lost." |

## 10. Interaction & motion
- Fade-in layout. Tapping "+ Add Medication" slides a new prescription card into the right workspace.
